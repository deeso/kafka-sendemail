...
class SendEmail(object):
    logger = Logger('kafka_email_service')
    ...
    def get_smtp_host(self):
        return getattr(self, SMTP_HOST)
    
    def get_smtp_port(self):
        return getattr(self, SMTP_PORT)

    def get_smtp_level(self):
        return getattr(self, SMTP_LEVEL)

    def get_smtp_use_tls(self):
        return getattr(self, SMTP_USE_TLS)

    def get_smtp_username(self):
        return getattr(self, SMTP_USERNAME)

    def get_smtp_password(self):
        return getattr(self, SMTP_PASSWORD)

    def send_email(self, sender, recipients, message):
        x = "Connecting to server"
        self.logger.action('service.send_mime_messages', sender, x)
        server = smtplib.SMTP(self.get_smtp_host(), self.get_smtp_port())
        server.set_debuglevel(self.get_smtp_level())

        if self.get_smtp_use_tls():
            x = "starting TLS"
            self.logger.action('service.send_email', sender, x)
            server.starttls()

        if self.get_smtp_username() is not None and self.get_smtp_password() is not None:
            x = "authenticating as: %s"% self.user
            self.logger.action('service.send_email', sender, x)
            server.login(self.get_smtp_username(), self.get_smtp_password())

        elif self.get_smtp_password() is not None:
            x = "authenticating as: %s"% sender
            self.logger.action('service.send_email', sender, x)

            server.login(sender, self.get_smtp_password())
    ...
    def add_attachments(self, message, attachment_name=None, attachment=None, filenames_contents=[]):
        ...
        for filename_content in filenames_contents:
            ...
            self.logger.action('service.add_attachments', 'email_sender', x)
    ...
    def send_mime_message(self, sender, recipient, cc=[], bcc=[],
        ...
        self.logger.action('service.send_mime_message', sender, x)
    ...
    def send_mime_zip(self, sender, recipient, cc=[], bcc=[], subject='', encoding='us-ascii',
        ...
        x = "Building mime message to: %s"% (recipient)
        self.logger.action('service.send_mime_zips', sender, x)
        message = MIMEMultipart()
        message['From'] = sender
        message['To'] = recipient
        message['Subject'] = subject
